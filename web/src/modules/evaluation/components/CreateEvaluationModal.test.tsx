...
const skillResource: ResourceSummary = {
...
describe('CreateEvaluationModal', () => {
  ...
  it('submits a published plan when the picked suite has a published revision', async () => {
    ...
    render(<CreateEvaluationModal open resources={[skillResource]} onClose={vi.fn()} onSubmit={onSubmit} />);
    ...
    expect(plan).toEqual({
      mode: 'published',
      resource: { kind: 'skill', resource_id: 'skill-1', revision_id: 'v1' },
      suiteId: 'suite-1',
      revisionId: 'rev-v2',
    });
  });
  ...
  it('submits a create plan whose case maps the authored fields', async () => {
    ...
    expect(plan.resource).toEqual({ kind: 'skill', resource_id: 'skill-1', revision_id: 'v1' });
  ...
  it('keeps the previous session hidden when reopening', async () => {
    ...
    const { rerender } = render(
      <CreateEvaluationModal open resources={[skillResource]} onClose={vi.fn()} onSubmit={onSubmit} />,
    );
    fireEvent.click(screen.getByRole('radio', { name: '新建评测集' }));
    await selectResource();
    ...
    rerender(<CreateEvaluationModal open={false} resources={[skillResource]} onClose={vi.fn()} onSubmit={onSubmit} />);
    rerender(<CreateEvaluationModal open resources={[skillResource]} onClose={vi.fn()} onSubmit={onSubmit} />);
